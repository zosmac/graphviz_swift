...
Note that to display Graphviz Attributes documentation, *GraphvizSwift* uses WebKit's WebView and WebPage, introduced with macOS 26 Tahoe. You can build for macOS 15 Sequoia by editing `GraphvizApp.swift` to use instead AttributesDocView, which is a NSViewControllerRepresentable that uses WKWebView.
...
Create the macOS package installer `graphvizswift-arm64.pkg` to install the GraphvizSwift App locally and to distribute it for install on other macOS hosts.
```zsh
cd graphviz_swift
make pkg
```
